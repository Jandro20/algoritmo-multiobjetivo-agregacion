...
// Vecindad (%) 20% de la poblacion total 
int const T = 20;
...
// Parametros de mutacion y cruce 
float const F = 0.5;
float const CR = 0.5;
...
// Funciones para operaciones con random
    double seed;
    double oldrand[55];
    int jrand;
...
// Inicializamos vectores peso con vectores equiespacioados de forma euclidea de 0.0 a 1.0
...
void evaluate_zdt3_all(float population[N][EB], float evaluation[N][2], float pReference[2])
{
    ...
    for (int individuo = 0; individuo < N; individuo++)
    {
        ...
        //Realizo sumatorio del espacio de busqueda del individuo
        for (int j = 1; j < EB; j++)
        {
    ...
    // Obtenemos los puntos de referencia globales
    pReference[0] = bestX;
    pReference[1] = bestY;  
}
...
void evaluate_zdt3(float individuo[EB], float evaluation[2], float pReference[2])
{
    ...
    //Realizo el sumatorio del espacio de busqueda del individuo
...
// Calculamos funcion objetivo mediante la formulacion de Tchbychef
...
void fmutation(float population[N][EB], int subproblema, int neightbours[N][T], float mutation[EB], Fichero fprueba){
    ...
    // Obtenemos 3 numneros randoms distintos entre ellos
    do
    {
        r1 = (int) (URAND*T);
	...
	// Evitamos que al realizar la mutación, nos salgamos del rango establecido (0-1)
        if(mutation[t] > 1.){ 
            mutation[t]=1.;
        }
        if(mutation[t] < 0.) {
...
// Funcion ppal donde se desarrollan todas las iteraciones del programa
void iteraciones(float population[N][EB], int neightbours[N][T], float pReferenceGlobal[2], float pesos[N][2],
    ...
    for (int iteracion = 0; iteracion < G; iteracion++)
    {
	    ...
	    // CRUCE
		    ...
		    // Mantenemos dentro del rango (0-1)
	    ...
	    // EVALUACION
            ...
            // ACTUALIZACION PUNTO REFERENCIA    
	    ...
	    // ACTUALIZAMOS VECINOS
        	...
        	valorActual = evaluacionGTE(posibleIndividuo, pesos[vecino], pReferenceGlobal);
             ...
             // Guardamos la población completa por cada iteracion en fpruebas   
             #ifdef degug
             for (int i = 0; i < N; i++)
             {
        	for (int j = 0; j < EB; j++)
	...
	// Obtenemos los mejores N valores obtenidos (frente paleto)
        float value1 = 0.;
        float value2 = 0.;
        int flag = 0;
	    ...
	    // Solo almacenamos el valor si los nuevos valores son mejores que los anteriores
            if(flag == 1){
